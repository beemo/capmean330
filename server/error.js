...
var debug = require('debug')('menstackjs:error')
var mongoose = require('mongoose')
var ErrorsModel = null

function checkError (err, cb) {
  if (err) {
    console.log('Error trying to record error', err.stack)
    cb && cb(err)
    return true
  }
}
...
function log (error, cb) {
  ...
  try {
    ...
    console.log('This Error happend before mongoose could set up or you have deleted model')
    console.log('This Uncaught Exceptions will not be tracked in the database')
    console.log('Reason:')
    console.log(e.stack)
